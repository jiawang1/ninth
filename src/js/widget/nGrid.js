(function(factory){
    ...
    var gridview = function (options) {        
        ...
        var leftCol, rightCol, maxLeft, maxRight, oLeftCol, oRightCol, startMoving = false;
        ...
        function onMousedown(event) {
            ...
            var $target = $(event.target||event.srcElement);
             ...
             if ($target.prevAll().length < 1&&width < 2 || $target.nextAll().length < 1&&($target.width() - width) < 2) {
                 return;
             }
             ...
             if (width < 2 || ($target.width() - width) < 2) {
                $target.css({ 'cursor': 'col-resize' });
                 startMove = true;
                              tSplitter.addClass("splitter");
                 tSplitter.css("left", event.clientX -  getLeft(tSplitter) + "px");
                 if (width < $target.width() / 2) {
                     leftCol =  $target.prev();
                     rightCol = currentTh = $target;
                 }
                 else {
                    leftCol = currentTh = $target;
                 ...
                 maxRight = $(rightCol).offset().left +$(rightCol).outerWidth();
        ...
        function onMousemove(event) {
             ...
             if(startMove === true && currentTh){
                ...
                if(event.clientX - maxLeft < 16 || maxRight - event.clientX < 16){
                    return;
                }
        ...
        function getLeft($ele){
                ...
                while($parent.length > 0&& $parent[0].tagName !== "HTML"){
                    actualLeft = actualLeft + $parent.position().left;
         ...
         function onMouseup(event) {
            ...
            if (startMove === true) {
                ...
                var leftWidth, rightWidth;
                    ...
                    leftWidth = tSplitter.offset().left - maxLeft;
                    rightWidth = maxRight - tSplitter.offset().left ;
                ...
                tSplitter.removeClass("splitter");
    ...
    // 
         ...
         function show() {
             ...
             gridContainer.append(tSplitter);
